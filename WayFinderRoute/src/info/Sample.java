...
public class Sample {
	...
	public int minDistance, distance, minRoute;
	...
	public void initStation() {
		s701 = createStation(701, 7);
		s406 = createStation(406, 4);
		...
		//
		s702 = createStation(702, 7);
		j401706.addStation(new StationDistance(s702, 4));

		//
		s438 = createStation(438, 4);
	...
	public Station createStation(int ID, int route) {
		Station newStation = new Station(ID, route);
	...
	public void testFindRoute() {
		findLeastRoute(s701, s406);
	}

	public void testFindRoute2() {
		findLeastRoute(s701, s702);
	}

	public void testFindRoute3() {
		findLeastRoute(s406, s438);
	}

	public void findLeastRoute(Station s1, Station s2) {
		if (s1.joins.size() == 0 || s2.joins.size() == 0) {
			System.out.println("Input problem");
			return;
		}
		...
		minDistance = DEFAULT_MIN_DISTANCE;
		minRoute = DEFAULT_MIN_DISTANCE;

		// same road or not ?
		if (inSameRoute(s1, s2)) {
			System.out.println(s1.ID + " " + s2.ID + " is on the same route");
			System.out.println();
		} else {

			dijkstraRoute(s1);
			lastCheckJoin = findMinRouteFromJoin(s2);
			//checkInitRouteFind();
			...
			CheckJoin temp = lastCheckJoin;
			while (temp != null) {
				System.out.print(" " + temp.join.ID + "(" + temp.curRoute + ")");
				temp = findCheckJoin(temp.lastJoin);
			}
	...
	public void dijkstraRoute(Station s1) {
		// update for join connect to station s1
		for (Join j1 : s1.joins) {
			CheckJoin cj = findCheckJoin(j1);
			for (StationDistance sd1 : j1.sd) {
				if (sd1.station == s1) {
					cj.distance = sd1.distance;
					cj.curRoute = s1.route;
					if (s1.route == 0) {
						// s1.route = 0 when the station is a join
						cj.numRoute = 0;
					} else {
						cj.numRoute = 1;
					}
	...
	public void dijkstraRouteJoin() {
		CheckJoin cj;
		while (remainJoins.size() > 0) {
			cj = findMinRouteCheckJoin();
			remainJoins.remove(cj.join);
			for (Join j : remainJoins) {
				for (JoinDistance jd : j.jd) {
					if (jd.join == cj.join) {
						CheckJoin temp = findCheckJoin(j);
						int tempNumRoute;
						if (cj.curRoute == jd.route) {
							tempNumRoute = cj.numRoute;
						} else {
							tempNumRoute = cj.numRoute + 1;
						}

						if (tempNumRoute < temp.numRoute
								|| (tempNumRoute == temp.numRoute && cj.distance + jd.distance < temp.distance)) {
							temp.lastJoin = cj.join;
							temp.distance = cj.distance + jd.distance;
							temp.numRoute = tempNumRoute;
							temp.curRoute = jd.route;
	...
	public CheckJoin findMinRouteCheckJoin() {
		int minDistance = DEFAULT_MIN_DISTANCE + 1;
		int minRoute = minDistance;
		CheckJoin temp = null;
		for (CheckJoin cj : completedJoins) {
			if (remainJoins.contains(cj.join)) {
				if (cj.numRoute < minRoute) {
					minRoute = cj.numRoute;
					minDistance = cj.distance;
					temp = cj;
				} else if (cj.numRoute == minRoute && cj.distance < minDistance) {
					minRoute = cj.numRoute;
					minDistance = cj.distance;
					temp = cj;
				}
	...
	public CheckJoin findMinRouteFromJoin(Station s2) {
		CheckJoin cj = null, temp;
		int tempNumRoute;
		for (Join j : s2.joins) {
			temp = findCheckJoin(j);
			if (temp.curRoute == s2.route) {
				tempNumRoute = temp.numRoute;
			} else {
				tempNumRoute = temp.numRoute + 1;
			}

			if (tempNumRoute < minRoute) {
				minRoute = tempNumRoute;
				cj = temp;
				for (StationDistance sd : j.sd) {
					if (sd.station == s2) {
						minDistance = sd.distance + temp.distance;
			...
			} else if (tempNumRoute == minRoute && cj != null) {
				for (StationDistance sd : j.sd) {
					if (sd.station == s2) {
						if (sd.distance + temp.distance < minDistance) {
							minDistance = sd.distance + temp.distance;
							cj = temp;
						}
					}
	...
	public boolean inSameRoute(Station s1, Station s2) {
		if (s1.joins.size() == s2.joins.size()) {
			int x = 0;
			for (Join j1 : s1.joins) {
				if (s2.joins.contains(j1)) {
	...
	public void checkInitRouteFind() {
		System.out.println(remainJoins.size() + " " + completedJoins.size());
		for (Join j : remainJoins) {
			System.out.println(j.ID);
		}
		for (CheckJoin cj : completedJoins) {
			if (cj.lastJoin != null) {
				System.out.println(cj.join.ID + " - " + cj.lastJoin.ID + ": " + cj.distance + ", " + cj.numRoute + ", "
						+ cj.curRoute);

			} else {
				System.out.println(cj.join.ID + " - null: " + cj.distance + ", " + cj.numRoute + ", " + cj.curRoute);
			}
		}
	}

	// end of find least route
